...
# %% COLUMNAS UTILES
columnasutiles = ["IDENTIFICACION DEL VEHICULO",
                  "CODIGO MARCA",
                  "TRANSMISION",
                  "TRACCION",
                  "Número Ejes",
...
costa_rica["CODIGO MARCA"] = costa_rica["CODIGO MARCA"].astype(str)
marcas["MARCA"] = marcas["MARCA"].astype(str)
costa_rica = pd.merge(
    costa_rica,
    marcas,
    left_on="CODIGO MARCA",
    right_on="MARCA",
    how="left"
    )
marcas = None
costa_rica["CODIGO MARCA"] = costa_rica["Nombre de marca"]
costa_rica = costa_rica[columnasutiles]
...
# %% QUITAR CARACTERES LATINOS
import unicodedata
cols = costa_rica.select_dtypes(include=[np.object]).columns
costa_rica[cols] = costa_rica[cols].apply(lambda x: x.str.normalize('NFKD').str.encode('ascii', errors='ignore').str.decode('utf-8'))


for column in list(costa_rica.columns):
    costa_rica[column] = costa_rica[column].astype(str).str.replace("|","/")
    costa_rica[column] = costa_rica[column].astype(str).str.replace("ñ","n")


costa_rica.columns = costa_rica.columns.str.replace(r"ñ", "n")
costa_rica.columns = costa_rica.columns.str.normalize('NFKD').str.encode('ascii', errors='ignore').str.decode('utf-8')


for column in list(costa_rica.columns):
    condicion = costa_rica[column].isin(["nan","None"])
    costa_rica.loc[condicion, column] = None


costa_rica.to_csv(r"D:\Basededatos\Limpioparaentregar\costa_rica_nolatino.csv", index=False)
